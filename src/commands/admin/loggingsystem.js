...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s.setName("remove").setDescription("Removes the audit logging system.")
    )
    .toJSON(),
      ...
      case "configure":
        ...
        if (!dataGD) {
          rEmbed
            ...
            .setDescription(
              "New server detetected: Configuring the audit logging system ..."
            );
        ...
        const auditTypeSSM = new ActionRowBuilder().setComponents(
          new StringSelectMenuBuilder({
            options: choices
              .slice(0, 14)
              .map((choice) => ({ label: choice.event, value: choice.event })),
          })
        ...
        rEmbed
          ...
          .setDescription(
            `What audit types related to \`Channels & messages\` should be logged and in which channel should every audit log action be sent to? \n\n You can change this anytime using the \`/loggingsystem configure\` command.`
          )
          .setFields(
            {
        ...
        SSMcollector.on("collect", async (i) => {
          ...
          auditTypeSSM.components[0].options = enabledAuditTypes.slice(
            -i.component.options.length
          );
        ...
        Btncollector.on("collect", async (i) => {
          ...
          switch (buttonClicks) {
            case 1:
              ...
              await loggingSchema.findOneAndUpdate(
                { GuildID: guildId },
                { Webhooks: webhookToDatabase, ChannelLogs: auditToDatabase }
              );
              break;
            case 2:
              ...
              await loggingSchema.findOneAndUpdate(
                { GuildID: guildId },
                { Webhooks: webhookToDatabase, GuildLogs: auditToDatabase }
              );
              break;
            case 3:
              ...
              await loggingSchema.findOneAndUpdate(
                { GuildID: guildId },
                { Webhooks: webhookToDatabase, MemberLogs: auditToDatabase }
              );
              break;
            case 4:
              ...
              break;
            case 5:
              const updatedData = await loggingSchema.findOneAndUpdate(
                { GuildID: guildId },
                { Webhooks: webhookToDatabase, OtherLogs: auditToDatabase },
              ...
              const clientGuildWebhooks = guildWebhooks.filter(
                (w) => w.name === client.user.username
              );
              let existingWebhookChannels = clientGuildWebhooks.map(
              ...
              for (let x = 0; x < existingWebhookChannels.length; x++) {
                if (updatedWebhookChannels.includes(existingWebhookChannels[x]))
                  continue;

                const removingChannel = guild.channels.cache.get(
                  existingWebhookChannels[x]
                );
                const removingChannelWebhooks =
                  await removingChannel.fetchWebhooks();
                const removingWebhook = removingChannelWebhooks.filter(
                  (w) => w.name === client.user.username
                );
          ...
          rEmbed
            ...
            .setColor(mConfig.embedColorWarning)
            .setDescription(
              `What audit types related to \`${auditCategory}\` should be logged and in which channel should every audit log action be sent to? \n\n You can change this anytime using the \`/loggingsystem configure\` command.`
            )
            .setFields(
              {
                name: "Logging channel",
