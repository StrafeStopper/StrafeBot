...
module.exports = async (client, auditLogEntry, guild) => {
    ...
    let auditEnabled;
    const logData = dataGD.ChannelLogs.find((arrayData) => arrayData.name === auditActionName);
    ...
    function getBinaryValues( bigIntValue, bitField) {
        ...
        async function convertAuditData(auditData, optionalValue, changes = false) {
            ...
            if (auditLogEntry.extra && logData === auditLogEntry.extra) {
                const fetchedMember = await guild.members.fetch(auditLogEntry.extra.id)
                    .catch(() => { return null });

                const fetchedRole = await guild.roles.fetch(auditLogEntry.extra.id)
                    .catch(() => { return null });

                const extraConversionObj = {
                    "id": () => `${fetchedMember ?? fetchedRole} - \`${logData[auditData]}\``,
                     "role_name": () => `${guild.roles.cache.get((role) => role.name === logData[auditData])}>`,
                     "type": () => (fetchedMember ? "`Member`" : "`Role`"),
                      "user": () => `${logData[auditData]}`,
                      "channel": () => `<#${logData[auditData].id}>`,
                      "messageId": () => `\`${logData[auditData]}\` - [Go to message](https://discord.com/channels/${guild.id}/${auditLogEntry.extra?.channel?.id}/${logData[auditData]})`,
                ...
                const mainConversionObj = {
                    ...
                    "permission_overwrites": () => (logData ? `\`${logData.join("\`, \`")}\`` : "`-`"),
                    "allow": () => `\`${getBinaryValues(BigInt(logData), "permissions").join("\`, \`") || "`-`"}\``,
                    "deny": () => `\`${getBinaryValues(BigInt(logData), "permissions").join("\`, \`") || "`-`"}\``,
                    ...
                    "default_auto_archive_duration": () => `\`${aConfig.archiveDuration[logData] || "-"}\``,
                    "flags": () => `\`${getBinaryValues(BigInt(logData), "channelFlags").join("\`, \`") || "`-`"}\``,
                    "default_reaction_emoji": () => `${guild.emojis.cache.get(logData.emoji_id) || logData.emoji_name}`,
                    "archived": () => (logData ? "`Yes`" : "`No`"),
                    "auto_archive_duration": () => `\`${aConfig.archiveDuration[logData || 0]}\``,
                    "autoArchiveDuration": () => `\`${aConfig.archiveDuration[logData || 0]}\``,
  ...
  const auditActionsObj = {
    ...
    "ChannelUpdate": [auditLogEntry.changes, ["available_tags", "template", "icon_emoji"], true],
    "ChannelDelete": [["name", "type"]],
  
    "ChannelOverwriteCreate": [["id*", "type*"]],
    "ChannelOverwriteUpdate": [["id*", "type*", ...auditLogEntry.changes], ["available_tags", "template", "icon_emoji"], true],
