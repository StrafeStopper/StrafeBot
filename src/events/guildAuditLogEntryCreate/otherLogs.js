...
module.exports = async (client, auditLogEntry, guild) => {
  ...
  let auditEnabled;
  const logData = dataGD.OtherLogs.find(
  ...
  async function convertAuditData(auditData, optionalValue, changes = false) {
    ...
    const mainConversionObj = {
      ...
      icon_hash: () =>
        `[image](https://cdn.discordapp.com/icons/${guild.id}/${logData}.${
          logData?.startsWith("a_") ? "gif" : "png"
        })` || "`No server icon`",
      splash: () =>
        `[image](https://cdn.discordapp.com/splashes/${guild.id}/${logData}.png)` ||
        "`No invite splash image`",
      discovery_splash: () =>
        `[image](https://cdn.discordapp.com/discovery-splashes/${guild.id}/${logData}.png)` ||
        "`No discovery splash image`",
      owner_id: () => `<@${logData}>`,
      afk_timeout: () =>
        `\`${logData / 60} ${logData === 60 ? "minute" : "minutes"}\``,
      widget_enabled: () => (logData ? "`Yes`" : "`No`"),
      verification_level: () => `\`${aConfig.verificationLevels[logData]}\``,
      default_message_notifications: () =>
        logData ? "`Only @mentions`" : "`All messages`",
      explicit_content_filter: () =>
        `\`${aConfig.explicitContentFilterLevels[logData]}\``,
      features: () => `\`${logData.join("`, `")}\``,
      mfa_level: () =>
        logData ? "`2FA requirement`" : "`No MFA/2FA requirement`",
      system_channel_flags: () =>
        `\`${
          getBinaryValues(BigInt(logData), "systemChannelFlags").join("`, `") ||
          "`-`"
        }\``,
      banner: () =>
        `[image](https://cdn.discordapp.com/banners/${guild.id}/${logData}.${
          logData?.startsWith("a_") ? "gif" : "png"
        })` || "`No server banner`",
  ...
  const auditActionsObj = {
    ...
    InviteCreate: [
      ["code", "temporary", "maxAge", "maxUses", "inviterId", "channelId"],
    ],
