...
module.exports = async (client, auditLogEntry, guild) => {
    ...
    let auditEnabled;
    const logData = dataGD.IntegrationlLogs.find((arrayData) => arrayData.name === auditActionName);
    ...
    const wEmbed = new EmbedBuilder()
        ...
        function getTriggerMetadata (metadata) {
            ...
            const metadataProperties = [
                "keywordFilter", "keyword_filter", "regexPatterns", "regex_patterns", "presets", "allowList", "allow_list",
                "mentionTotalLimit", "mention_total_limit", "mentionRaidProtectionEnabled", "mention_raid_protection_enabled"
              ];
              ...
              for (let metadataProperty of metadataProperties) {
                ...
                if (MetadataProperty === "presets") {
                    for (let i = 0; i < propertyData.length; i++) {
                        propertyData[i] = `\`${aConfig.keywordPresetTypes[propertyData[i]]}\``
                    };
                };

                if (["mentionTotalLimit", "mention_total_limit"].includes(metadataProperty)) {
                    propertyData = [`${metadata.mentionTotalLimit ?? metadata.mention_total_limit ?? 0} unique mentions`];
                    metadataProperty = "mentionTotalLimit";
                };

                if (["mentionRaidProtectionEnabled", "mention_raid_protection_enabled"].includes(metadataProperty)) {
                    propertyData = [metadata.mentionRaidProtectionEnabled ?? metadata.mention_raid_protection_enabled ? "`Yes`" : "`No`"];
                    metadataProperty = "mentionRaidProtectionEnabled";
                };

                result.push([`**${metadataProperty}:** \`${propertyData.join("\`, \`") || "-"}\``]);
            };
        ...
        function getActions(actions) {
            ...
            for (let action of actions) {
                ...
                switch (action.type) {
                    case 1:
                        actionMetadata = `*${(action.metadata.custom_message ?? action.metadata.customMessage) || "No custom message"}*`;
                        break;
                    case 2:
                        actionMetadata = `<#${action.metadata.channel_id ?? action.metadata.channelId}>`;
                        break;
                    case 3:
                        actionMetadata = `${aConfig.timeoutDuration[action.metadata.duration_seconds] ?? aConfig.timeoutDuration[action.metadata.durationSeconds]}`;
                        break;
                };
        ...
        async function convertAuditData(auditData, optionalValue, changes = false) {
            ...
            if (auditLogEntry.extra && logData === auditLogEntry.extra) {
                const extraConversionObj = {
                    ...
                    "autoModerationRuleTriggerType": () => `\`${aConfig.triggerTypes[logData[auditData]]}\``,
                ...
                if (!isNan(auditData)) {
                    convertedData = logData?.permission ? "`✅ Allowed`" : "`❌ Disallowed`";
                    return convertedData;
                ...
                const mainConversionObj = {
                    ...
                    "exemptRoles": () => (logData.size ? `<@&${logData.map((role) => role.id).join(">, <@&")}>` : "`No roles`"),
                    "exempt_roles": () => (logData.length ? `<@&${logData.join(">, <@&")}>` : "`No roles`"),
                    "exemptChannels": () => (logData.size ? `<#${logData.map((channel) => channel.id).join(">, <#")}>` : "`No channels`"),
                    "exempt_channels": () => (logData.length ? `<#${logData.join(">, <#")}>` : "`No channels`"),
                  
                    "avatar": () => (logData ? `[image](https://cdn.discordapp.com/avatars/${auditLogEntry.targetId}/${logData}.png)` : "`No avatar icon`"),
                    "avatar_hash": () => (logData ? `[image](https://cdn.discordapp.com/avatars/${auditLogEntry.targetId}/${logData}.png)` : "`No avatar icon`"),
                ...
                } else if (auditData.startsWith("$add_") || auditData.startsWith("$remove_")) {
                    convertedData = `\`${logData?.join("\`, \`") || "-"}\``;
                } else if (auditData === "type") {
                    if (auditLogEntry.targetType === "Integration") {
                        convertedData = `\`${logData.charAt(0).toUpperCase() + logData.slice(1).replaceAll("_", " ")}\``;
                    } else {
                ...
                if (typeof auditDataType === "string") {
                    ...
                    if (auditDataType.includes("*")) {
                        ...
                        if (["triggerMetadata", "actions"].includes(auditDataType)) {
                            convertAndAddField(`${auditDataType.charAt(0).toUpperCase() + auditDataType.slice(1).replaceAll("_", " ")}`, fieldValue || "`-`");
                        } else {
    ...
    "ApplicationCommandPermissionUpdate": [["targetId+", ...auditLogEntry.changes], [], true],

    "AutoModerationRuleCreate": [["id", "name", "triggerType", "eventType", "triggerMetadata", "actions", "enabled", "exemptRoles", "exemptChannels"]],
    "AutoModerationRuleUpdate": [["id", ...auditLogEntry.changes], [], true],
    "AutoModerationRuleDelete": [["id", "name"]],
    "AutoModerationBlockMessage": [["targetId+", "reason+", "channel*", "autoModerationRuleName*", "autoModerationRuleTriggerType*"]],
    "AutoModerationFlagToChannel": [["targetId+", "autoModerationRuleName*"]],
    "AutoModerationUserCommunicationDisabled": [["targetId+", "autoModerationRuleName*"]],
