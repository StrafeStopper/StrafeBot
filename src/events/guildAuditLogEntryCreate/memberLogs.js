...
module.exports = async (client, auditLogEntry, guild) => {
  ...
  let auditEnabled;
  const logData = dataGD.MemberLogs.find(
  ...
  async function convertAuditData(auditData, optionalValue, changes = false) {
    ...
    if (auditLogEntry.extra && logData === auditLogEntry.extra) {
      ...
      const mainConversionObj = {
        ...
        premium_since: () =>
          logData
            ? `<t:${parseInt(logData.toString().slice(0, -3))}>`
            : "`Not a server booster`",
        deaf: () => (logData ? "`Yes`" : "`No`"),
        mute: () => (logData ? "`Yes`" : "`No`"),
        flags: () =>
          `\`${
            getBinaryValues(BigInt(logData), "guildMemberFlags").join("`, `") ||
            "`-`"
          }\``,
        pending: () =>
          logData
            ? "`Yes - Member has not passed the Membership Screening requirements of the guild`"
            : "`No - Member has passed the Membership Screening requirements of the guild`",
    ...
    for (const auditDataType of auditDataTypes) {
      ...
      if (changes) {
        ...
        if (["$add", "$remove"].includes(key)) {
          ...
          convertAndAddField(
            "Role(s)",
            await convertAuditData(key, newValue, true),
            true
          );
        } else {
          convertAndAddField(
          ...
          if (key.startsWith("communication")) {
            convertAndAddField(
              "Timeout active until",
              await convertAuditData(key, trueValue, true),
              true
            );
          } else {
            convertAndAddField(
