...
module.exports = async (client, auditLogEntry, guild) => {
    ...
    let auditEnabled;
    const logData = dataGD.GuildLogs.find((arrayData) => arrayData.name === auditActionName);
    ...
    function decimalToHex(decimal) {
        ...
        async function convertAuditData(auditData, optionalValue, changes = false) {
            ...
            const mainConversionObj = {
                ...
                "scheduledStartTimestamp": () => (logData ? `<t:${parseInt(logData.toString().slice(0, -3))}>, which is <t:${parseInt(logData.toString().slice(0, -3))}:R>` : "`No specified start time`"),
                "scheduledEndTimestamp": () => (logData ? `<t:${parseInt(logData.toString().slice(0, -3))}>, which is <t:${parseInt(logData.toString().slice(0, -3))}:R>` : "`No specified end time`"),
                ...
                "image": () => `[image](https://cdn.discordapp.com/guild-events/${auditLogEntry.target.id}/${logData}.png)` || "`No cover image`",
              
                "type": () => `\`${aConfig.stickerTypes[logData]}\``,
                "format": () => `\`${aConfig.stickerFormats[logData]}\``,
              
                "icon": () => `[icon](https://cdn.discordapp.com/role-icons/${auditLogEntry.target.id}/${logData}.png)` || "`No role icon`",
                "color": () => `\`${decimalToHex(logData)}\``,
                "hoist": () => (logData ? "`Yes`" : "`No`"),
                "mentionable": () => (logData ? "`Yes`" : "`No`"),
                "permissions": () => `\`${getBinaryValues(BigInt(logData), "permissions").join("\`, \`") || "`-`"}\``,
                "flags": () => `\`${[logData].bitfield ? "Can be selected in onboarding" : "Cannot be selected in onboarding"}\``,
        ...
        const auditActionsObj = {
            ...
            "GuildScheduledEventCreate": [["name", "description", "id", "channelId", "scheduledStartTimestamp", "scheduledEndTimestamp", "privacyLevel", "status", "entityType"]],
            "GuildScheduledEventUpdate": [["id", "scheduledStartTimestamp", "scheduledEndTimestamp", ...auditLogEntry.changes], [], true],
